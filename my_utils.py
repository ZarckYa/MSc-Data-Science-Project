...
def compute_init_ability(Answers):
    ...
    return math.ceil(initial_ability*100)/100


def generate_DeepIRT_data(respones_vector, selected_items, quesdiff, part_num):
    data_list = []
    respones_vector_0_1 = []
    len_keep = 0
    for res in respones_vector:
        if res:
            respones_vector_0_1.append(1)
        else:
            respones_vector_0_1.append(0)

    for ind, i in enumerate(selected_items):
        temp = []
        temp = [i] * len(quesdiff[i])
        data_list.append([len(quesdiff[i])])
        data_list.append(temp)
        if ind == 0:
            data_list.append(list(respones_vector_0_1[len_keep:len(quesdiff[i])]))
        else:
            data_list.append(list(respones_vector_0_1[len_keep:]))
        len_keep = len(quesdiff[i])
    train_path = "DeepIRT/data/mydata/my_data_train" + str(part_num) + ".csv"
    valid_path = "DeepIRT/data/mydata/my_data_valid" + str(part_num) + ".csv"
