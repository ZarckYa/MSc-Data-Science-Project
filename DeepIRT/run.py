...
def run_model(model, args, q_data, qa_data, mode='train'):
    ...
    for step in range(training_step):
        ...
        if mode == "train":
            ...
            ability_value = model.sess.run([model.student_abilities], feed_dict=feed_dict)
            diff_value = model.sess.run([model.question_difficulties], feed_dict=feed_dict)
        else:
        ...
        abilities_value_arr = np.asarray(ability_value).reshape((-1))
        diff_value_arr = np.asarray(diff_value).reshape((-1))

        label_list.append(label_flat[index_flat])
        pred_list.append(pred_flat[index_flat])
        abilities_list.append(abilities_value_arr[index_flat])
        diff_list.append(diff_value_arr[index_flat])
    ...
    all_abilities = np.concatenate(abilities_list, axis=0)
    all_diff = np.concatenate(diff_list, axis=0)

    # auc = compute_auc(all_label, all_pred)
    # accuracy = compute_accuracy(all_label, all_pred)
    loss = binaryEntropy(all_label, all_pred)

    # return loss, accuracy, auc, all_abilities, all_diff
    return loss, all_abilities, all_diff